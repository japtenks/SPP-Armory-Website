 ...
 * - Step 1: Grab the itemset row from dbc_itemset
 * - Step 2: For each item_1..10, look up item_template → displayid
 *           Then resolve displayid → inventoryIcon using dbc_itemdisplayinfo
 * - Step 3: For each bonus, pull spell info from dbc_spell
 */
...
/**
 * Resolve displayid → inventoryIcon (from dbc_itemdisplayinfo).
 * Falls back to inv_misc_questionmark if missing.
 */
function icon_from_displayid(int $displayId): string {
   ...
   * - Sorts bonuses by pieces required.
   * - Runs descriptions through token replacer.
   */
...
/* ---------- spell tooltip parser (from talent-calc) ---------- */
...
if (!function_exists('replace_spell_tokens')) {
  function replace_spell_tokens(string $desc, array $sp): string {
    // $s1, $s2, $s3 → effect_basepoints_N + 1
    for ($i=1; $i<=3; $i++) {
...
//spell effects, Hit Rating, etc
function render_spell_effect(int $spellId, int $trigger, array $row = []): string {
    // Ratings (virtual spell effects from stat_type/stat_value)
    if ($spellId === 0 && !empty($row)) {
        static $ratingMap = [
          12=>'Defense Rating',
          13=>'Dodge Rating',
          14=>'Parry Rating',
...
function render_item_tip_html(array $item): string {
    // Extended query: include spellid fields
    $sql = "
	...
	// 2. Resistances (always after stats)
	$resMap = [
	...
	for ($i=1; $i<=5; $i++) {
		...
		// Classes (use your mask → names function)
...
foreach ($order as $key) {
  ...
  // normalize: if no variants, treat it as a 1-element array
  if (!$pairs) {
    $nmRaw = $N[$key][$selectedClass] ?? '';
    if ($nmRaw !== '') {
      $pairs = [['name' => $nmRaw, 'role' => '']];
    }
  }
    ...
    if ($setId) {
      ...
      if (!empty($data['items'])) {
        ...
        foreach ($data['items'] as $it) {
          ...
          . 'class="js-item-tip" '
          . 'data-item-id="' . (int)$it['entry'] . '" '
          . 'data-tip-html="' . htmlspecialchars($tipItemHtml, ENT_QUOTES) . '">'
...
.set-row {
  display: flex;
  align-items: center;
  justify-content: flex-start;   /* name on left, icons on right */
  margin: 6px 0;
}

.set-name {
  flex: 0 0 240px;               /* reserve consistent space for names */
  font-weight: bold;
}
...
<script>
(function(){
  // reuse the same tooltip div
  const tip = document.querySelector('.talent-tt') || (function(){
    const d = document.createElement('div');
    d.className = 'talent-tt';
  ...
  function show(el){
    ...
    const raw = el.getAttribute('data-tip-html') || '';
    const ta  = document.createElement('textarea');
    ta.innerHTML = raw;         // decode
    tip.innerHTML = ta.value;   // real HTML
    place(el);
  }
  function hide(){ tip.style.display = 'none'; anchor = null; }
  function nudge(){ if(anchor && tip.style.display !== 'none') place(anchor); }

  // Bind both set tooltips and item tooltips
  document.addEventListener('mouseover', e=>{
