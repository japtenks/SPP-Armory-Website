...
function talent_bg_for_tab($tabId) {
  ...
  $fsBase  = realpath(__DIR__ . '/../images/icon_talents');
  if (!$fsBase) return '';
  $file = (int)$tabId . '.jpg';
...
<head>
  ...
  <?php
    $cssPath = $_SERVER['DOCUMENT_ROOT'].'/templates/offlike/css/talents.css';
    $jsPath  = $_SERVER['DOCUMENT_ROOT'].'/templates/offlike/js/talents.js';
  ?>
  <link rel="stylesheet" href="/templates/offlike/css/talents.css<?= is_file($cssPath) ? '?v='.filemtime($cssPath) : '' ?>">
  <?php if (!$isProfileTalentView): ?>
  <script defer src="/templates/offlike/js/talents.js<?= is_file($jsPath) ? '?v='.filemtime($jsPath) : '' ?>"></script>
  <?php endif; ?>
</head>
<body>
